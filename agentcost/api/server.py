...
if _ENTERPRISE:
    ...
    from ..community_auth import (
        get_auth_config, get_optional_user,
        require_role, AuthContext, AuthMiddleware,
    )
...
async def _auto_migrate():
    ...
    _apply_sql_dir(db, core_mig, skip_files={"002_enterprise.sql"} if not _ENTERPRISE else set())
...
def get_events():
    global _events
    if not _events: _events = EventStore()
    return _events

def get_benchmarks():
    global _benchmarks
    if not _benchmarks: _benchmarks = BenchmarkStore()
    return _benchmarks
...
if os.path.isdir(_dashboard_js_dir):
    app.mount("/dashboard/js", StaticFiles(directory=_dashboard_js_dir), name="dashboard-js")
...
@app.get("/api/summary")
async def summary(project: str | None = None, user: AuthContext = Depends(get_optional_user)):
...
async def cost_by_model(project: str | None = None, user: AuthContext = Depends(get_optional_user)):
...
@app.post("/api/trace")
async def ingest_trace(request: Request, user: AuthContext = Depends(get_optional_user)):
    from ..sdk.trace import TraceEvent
    data = await request.json()
    event = TraceEvent(
        trace_id=data.get("trace_id", ""), project=data.get("project", "default"),
        model=data.get("model", "unknown"), provider=data.get("provider", "unknown"),
        input_tokens=data.get("input_tokens", 0), output_tokens=data.get("output_tokens", 0),
        cost=data.get("cost", 0), latency_ms=data.get("latency_ms", 0),
        status=data.get("status", "success"), error=data.get("error"),
...
async def ingest_trace_batch(request: Request, user: AuthContext = Depends(get_optional_user)):
    from ..sdk.trace import TraceEvent
...
async def trace_count(project: str | None = None, user: AuthContext = Depends(get_optional_user)):
...
async def set_budget(
    project: str, daily: float | None = None, monthly: float | None = None,
    total: float | None = None, user: AuthContext = Depends(get_optional_user),
):
...
async def benchmark_runs(limit: int = 50, user: AuthContext = Depends(get_optional_user)):
...
async def cost_forecast(project: str, days: int = 30, method: str = "ensemble",
                        user: AuthContext = Depends(get_optional_user)):
...
async def budget_exhaustion(project: str, budget: float = 100.0,
                            user: AuthContext = Depends(get_optional_user)):
    from ..forecast import CostForecaster
    traces = get_events().get_traces(project, limit=10000)
    forecaster = CostForecaster()
...
async def estimate_cost(request: Request, user: AuthContext = Depends(get_optional_user)):
    from ..estimator import CostEstimator
    body = await request.json()
    estimator = CostEstimator()
    model = body.get("model", "gpt-4o")
    if "messages" in body:
        est = estimator.estimate_messages(model, body["messages"],
              task_type=body.get("task_type", "default"),
              max_output_tokens=body.get("max_output_tokens"))
    else:
        est = estimator.estimate(model, body.get("prompt", ""),
              task_type=body.get("task_type", "default"),
              max_output_tokens=body.get("max_output_tokens"))
    return est.to_dict()

@app.get("/api/estimate/compare")
async def estimate_compare(prompt: str = "Hello", task_type: str = "default",
                           user: AuthContext = Depends(get_optional_user)):
    from ..estimator import CostEstimator
    estimator = CostEstimator()
    popular = ["gpt-4o", "gpt-4o-mini", "claude-3-5-sonnet", "claude-3-5-haiku",
               "gemini-1.5-pro", "gemini-1.5-flash", "llama3:8b"]
    return estimator.compare_models(prompt, popular, task_type)

@app.get("/api/analytics/{project}/summary")
async def analytics_summary(project: str, user: AuthContext = Depends(get_optional_user)):
...
async def analytics_top_spenders(project: str, by: str = "model", limit: int = 10,
                                 user: AuthContext = Depends(get_optional_user)):
    from ..analytics import UsageAnalytics
    analytics = UsageAnalytics()
    analytics.add_traces(get_events().get_traces(project, limit=10000))
...
async def analytics_efficiency(project: str, user: AuthContext = Depends(get_optional_user)):
    from ..analytics import UsageAnalytics
    analytics = UsageAnalytics()
    analytics.add_traces(get_events().get_traces(project, limit=10000))
...
async def analytics_chargeback(project: str, group_by: str = "project",
                               user: AuthContext = Depends(get_optional_user)):
    from ..analytics import UsageAnalytics
    analytics = UsageAnalytics()
    analytics.add_traces(get_events().get_traces(project, limit=10000))
...
async def optimizer_report(project: str, user: AuthContext = Depends(get_optional_user)):
...
if _ENTERPRISE:
    ...
    async def list_users(user: AuthContext = Depends(require_role("admin"))):
        ...
        rows = db.fetch_all(
            ...
            "WHERE org_id = ? ORDER BY created_at DESC", (user.org_id,))
    ...
    async def create_api_key(name: str = "Default", scopes: str = "*",
        ...
        db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (key_id, user.org_id, key_prefix, key_hash, name, scopes,
             user.sub, datetime.utcnow().isoformat()))
        from ..org.audit_service import AuditService
        AuditService(db).log_api_key_event(user.org_id, user.sub, key_id, "create")
        return {"id": key_id, "key": full_key, "prefix": key_prefix, "name": name,
                "scopes": scopes, "message": "Save this key — it will not be shown again."}
    ...
    async def list_api_keys(user: AuthContext = Depends(require_role("admin"))):
        ...
        rows = db.fetch_all(
            ...
            "FROM api_keys WHERE org_id = ? ORDER BY created_at DESC", (user.org_id,))
        return [dict(r) for r in rows]

    @app.delete("/api/admin/api-keys/{key_id}")
    async def revoke_api_key(key_id: str, user: AuthContext = Depends(require_role("admin"))):
        from ..data.connection import get_db
        db = get_db()
        db.execute("DELETE FROM api_keys WHERE id = ? AND org_id = ?", (key_id, user.org_id))
        from ..org.audit_service import AuditService
    ...
    async def event_subscribe(request: Request, user: AuthContext = Depends(get_optional_user)):
        from ..events import get_event_bus
        ...
        sub_id = get_event_bus().subscribe_webhook(url, body.get("event_types", ["*"]), body.get("secret", ""))
...
@app.post("/api/seed")
async def seed_sample_data(request: Request, user: AuthContext = Depends(get_optional_user)):
    ...
    projects_map = {
        ...
        "customer-support": ["ticket-classifier","response-drafter","escalation-agent"],
    ...
    hourly_w = [1,1,1,1,1,2,4,8,12,15,14,13,10,14,15,14,12,10,7,5,3,2,2,1]
    ...
    for day_off in range(days):
        ...
        dow = day.weekday()
        day_calls = max(10, int(base_calls * dow_mult[dow] * (1.03 ** day_off)) + _rand.randint(-15,15))
        for _ in range(day_calls):
            hour = _rand.choices(range(24), weights=hourly_w, k=1)[0]
            ts = day.replace(hour=hour, minute=_rand.randint(0,59), second=_rand.randint(0,59))
            mid, prov, inp, outp, avg_i, avg_o, _ = _rand.choices(
                models, weights=[m[6] for m in models], k=1)[0]
            if project_filter:
                proj = project_filter
            ...
            if "opus" in mid or "pro" in mid: base_lat *= 1.8
            elif "haiku" in mid or "nano" in mid or "flash" in mid: base_lat *= 0.5
            lat = max(50, base_lat * _rand.uniform(0.6, 1.5))
            status = "error" if _rand.random() < 0.03 else "success"
            err = _rand.choice(["Rate limit exceeded","Context length exceeded",
                                "Connection timeout","Internal server error"]) if status == "error" else None
            if status == "error":
                cost = 0; o_tok = 0
            event = TraceEvent(
                trace_id=_uuid.uuid4().hex[:12], project=proj, model=mid, provider=prov,
                input_tokens=i_tok, output_tokens=o_tok, cost=round(cost, 8),
                latency_ms=round(lat, 1), status=status, error=err,
                agent_id=agent,
    ...
    return {"status": "ok", "seeded": count, "days": days,
            "total_cost": round(total_cost, 2), "cleared": clear,
...
def run_server(host="0.0.0.0", port=None):
    ...
    if _ENTERPRISE:
        print(f"   Auth:      {'✅ Enabled (Keycloak)' if auth_enabled else '⚠️  Disabled'}")
        if auth_enabled:
