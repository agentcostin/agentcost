...
def cmd_leaderboard(args):
    ...
    for row in leaderboard:
        print(f"  {row['model']:<32}{row['total_runs']:<6}"
...
def cmd_traces(args):
    ...
    if args.summary:
        ...
        if not s or s.get("total_calls", 0) == 0:
            ...
            return
        print(f"\n{'='*60}")
        print("  📊 Cost Summary" + (f" — {args.project}" if args.project else " — All Projects"))
        print(f"{'='*60}")
        ...
        if models:
            print("\n  By Model:")
            for m in models:
                print(f"    {m['model']:<32} {m['calls']} calls  ${m['total_cost']:.6f}")
        print()
        return
    ...
    for t in traces:
        ...
        print(f"  {ts_short:<10}{t.get('model','?'):<28}{tokens:<18}"
...
def cmd_plugin(args):
    ...
    if sub == "list":
        ...
        for p in all_plugins:
            ...
            print(f"{p['name']:<25} {p['version']:<10} {p['type']:<12} {icon:<8} {p.get('description','')[:30]}")
        print()

    elif sub == "install":
        import subprocess
        pkg = f"agentcost-{args.name}" if not args.name.startswith("agentcost-") else args.name
        print(f"\n📦 Installing {pkg}...")
...
def cmd_info(args):
    ...
    print("\n🧮 AgentCost v1.0.0")
    print(f"   Edition: {'🏢 Enterprise' if edition == 'enterprise' else '🌐 Community'}")
...
def cmd_license(args):
    ...
    if action == "status":
        ...
        if lic.expires_at:
            print(f"   Expires:  {lic.expires_at.strftime('%Y-%m-%d')} ({lic.days_remaining} days)")
        else:
            print("   Expires:  Never")
        if lic.features:
        ...
        print(f"   Tier:     {lic.tier}")
        print(f"   User:     {lic.licensed_to}")
        print(f"   Expires:  {lic.expires_at.strftime('%Y-%m-%d') if lic.expires_at else 'Never'}")
        ...
        if os.environ.get("AGENTCOST_LICENSE_KEY"):
            print("⚠️  AGENTCOST_LICENSE_KEY env var is still set. Unset it to fully deactivate.")
...
Examples:
  ...
  agentcost budget my-app                      # Check budget status
    ...
    p_bench.add_argument("--model", "-m", default="gpt-4o", help="Model name (default: gpt-4o)")
    p_bench.add_argument("--tasks", "-t", type=int, default=5, help="Number of tasks (default: 5)")
    ...
    p_bench.add_argument("--eval-model", default="gpt-4o-mini", help="Model for quality evaluation")
    p_bench.add_argument("--provider", default="openai",
    ...
    p_bench.add_argument("--base-url", default=None,
                         help="Base URL for proxy/litellm-proxy (e.g. http://localhost:4000)")
    p_bench.add_argument("--ollama-url", default=None,
                         help="Ollama server URL (default: http://10.166.73.108:11434). Shortcut for --provider ollama --base-url URL")
    p_bench.add_argument("--no-verify-ssl", action="store_true", default=False,
                         help="Disable SSL certificate verification (for corporate gateways with internal CAs)")
    p_bench.add_argument("--output", "-o", default=None, help="Save markdown report to file")

    # compare
    p_comp = sub.add_parser("compare", help="Compare multiple models head-to-head")
    p_comp.add_argument("--models", "-m", required=True, help="Comma-separated model names")
    p_comp.add_argument("--tasks", "-t", type=int, default=5, help="Tasks per model (default: 5)")
    ...
    p_comp.add_argument("--eval-model", default="gpt-4o-mini", help="Model for quality evaluation")
    p_comp.add_argument("--provider", default="openai",
                        help="LLM provider: openai, anthropic, ollama, litellm, or proxy")
    p_comp.add_argument("--api-key", default=None, help="API key (overrides env var)")
    p_comp.add_argument("--base-url", default=None,
                        help="Base URL for proxy/litellm-proxy (e.g. http://localhost:4000)")
    p_comp.add_argument("--ollama-url", default=None,
                        help="Ollama server URL (default: http://10.166.73.108:11434). Shortcut for --provider ollama --base-url URL")
    p_comp.add_argument("--no-verify-ssl", action="store_true", default=False,
                        help="Disable SSL certificate verification (for corporate gateways with internal CAs)")
    p_comp.add_argument("--output", "-o", default=None, help="Save markdown report to file")
    ...
    p_dash.add_argument("--port", type=int, default=int(os.environ.get("AGENTCOST_PORT", "8500")), help="Port (default: 8500 or AGENTCOST_PORT)")
    ...
    p_traces.add_argument("--limit", "-n", type=int, default=50, help="Number of traces (default: 50)")
    p_traces.add_argument("--summary", action="store_true", help="Show cost summary instead of individual traces")

    # budget (Phase 2)
    p_budget = sub.add_parser("budget", help="View or set project budgets")
    p_budget.add_argument("project", help="Project name")
    p_budget.add_argument("--set", dest="set_limit", type=float, default=None, help="Set total budget limit in dollars")
    ...
    p_plugin_install = plugin_sub.add_parser("install", help="Install a plugin from PyPI")
    p_plugin_install.add_argument("name", help="Plugin name (e.g. slack-alerts)")
    p_plugin_create = plugin_sub.add_parser("create", help="Scaffold a new plugin project")
    p_plugin_create.add_argument("name", help="Plugin name")
    p_plugin_create.add_argument("--type", dest="plugin_type", default="notifier",
    ...
    p_gateway.add_argument("--port", "-p", type=int, default=8200, help="Port (default: 8200)")
    ...
    p_license.add_argument("license_action", nargs="?", default="status",
                           choices=["status", "trial", "activate", "deactivate"],
                           help="License action (default: status)")
    p_license.add_argument("key", nargs="?", default=None, help="License key (for activate)")
    p_license.add_argument("--days", type=int, default=30, help="Trial duration in days (default: 30)")
