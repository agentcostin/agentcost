...
class HeartbeatTracker:
    ...
    def __init__(self, db=None, anomaly_multiplier: float = 2.0,
    ...
    def start_cycle(self, agent_id: str) -> str:
        ...
        self.db.execute(
            """INSERT INTO heartbeat_cycles (id, agent_id, started_at, ended_at,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (cycle.cycle_id, agent_id, cycle.started_at, 0, 0, 0, "active", "", "default"),
        )
    ...
    def end_cycle(self, agent_id: str) -> dict | None:
        ...
        self.db.execute(
            ...
            (cycle.ended_at, cycle.cost, cycle.calls,
             cycle.status, cycle.anomaly_reason, cycle.cycle_id),
        )
    ...
    def reset(self, agent_id: str | None = None) -> None:
        if agent_id:
            ...
            self._paused_agents.discard(agent_id)
            self.db.execute("DELETE FROM heartbeat_cycles WHERE agent_id=?", (agent_id,))
            self.db.execute("DELETE FROM heartbeat_budgets WHERE agent_id=?", (agent_id,))
        else:
    ...
    def _emit_budget_event(self, event_type: str, agent_id: str, spend: float, budget: float):
        try:
            ...
            get_event_bus().emit(event_type, {
                "agent_id": agent_id, "spend": round(spend, 4),
                "budget": round(budget, 4),
                "usage_pct": round(spend / budget * 100, 1), "source": "heartbeat",
            })
        except Exception:
            pass
    ...
    def _emit_anomaly(self, agent_id: str, cycle: HeartbeatCycle):
        try:
            from ..events import get_event_bus
            get_event_bus().emit("anomaly.cost_spike", {
                "agent_id": agent_id, "cycle_id": cycle.cycle_id,
                "cost": round(cycle.cost, 6),
                "reason": cycle.anomaly_reason, "source": "heartbeat",
            })
        except Exception:
            pass
    ...
    def _row_to_cycle(self, row) -> HeartbeatCycle:
        return HeartbeatCycle(
            cycle_id=row["id"], agent_id=row["agent_id"],
            started_at=row["started_at"], ended_at=row.get("ended_at", 0),
            cost=row.get("cost", 0), calls=row.get("calls", 0),
