...
_OPERATORS = {
    ...
    "in": lambda v, t: v in t, "not_in": lambda v, t: v not in t,
...
class Reaction:
    ...
    def from_dict(cls, name: str, d: dict) -> "Reaction":
        ...
        return cls(
            name=name, auto=d.get("auto", True), actions=actions,
            ...
            retries=d.get("retries", 0), enabled=d.get("enabled", True),
        )
...
class ReactionResult:
    ...
    def to_dict(self) -> dict:
        return {
            "reaction": self.reaction_name, "event": self.event_type,
            "executed": self.actions_executed, "failed": self.actions_failed,
            "skipped": self.skipped_reason, "success": self.success,
...
def _builtin_defaults() -> dict[str, Reaction]:
    return {
        "budget-exceeded": Reaction(name="budget-exceeded", auto=True,
                                     actions=["notify", "log", "block-calls"], cooldown_seconds=1800),
        "budget-80": Reaction(name="budget-80", auto=True, actions=["notify", "log"],
                               condition={"usage_pct": {"gte": 80}}, cooldown_seconds=3600),
        "policy-violation": Reaction(name="policy-violation", auto=True,
                                      actions=["log", "notify"], cooldown_seconds=300),
        "cost-spike": Reaction(name="cost-spike", auto=True,
                                actions=["notify", "log"], cooldown_seconds=900),
    }


EVENT_TO_REACTION = {
    "budget.warning": "budget-80", "budget.exceeded": "budget-exceeded",
    "policy.violation": "policy-violation", "policy.blocked": "policy-blocked",
    "anomaly.cost_spike": "cost-spike", "anomaly.error_burst": "error-burst",
    "anomaly.token_explosion": "token-explosion", "anomaly.latency": "latency-anomaly",
    "anomaly.detected": "cost-spike", "approval.pending": "approval-pending",
    "approval.decided": "approval-decided", "agent.suspended": "agent-suspended",
    "agent.resumed": "agent-resumed", "scorecard.generated": "scorecard-generated",
}
...
class ReactionEngine:
    ...
    def _get_cooldown(self, name: str) -> float:
        row = self.db.fetch_one(
            "SELECT last_fired_at FROM reaction_cooldowns WHERE reaction_name=?", (name,))
        return row["last_fired_at"] if row else 0

    def _set_cooldown(self, name: str, ts: float):
        existing = self.db.fetch_one(
            "SELECT reaction_name FROM reaction_cooldowns WHERE reaction_name=?", (name,))
        if existing:
            self.db.execute(
                "UPDATE reaction_cooldowns SET last_fired_at=? WHERE reaction_name=?", (ts, name))
        else:
            self.db.execute(
    ...
    def _persist_result(self, result: ReactionResult):
        self.db.execute(
            """INSERT INTO reaction_history (reaction_name, event_type, actions_executed,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (result.reaction_name, result.event_type,
             json.dumps(result.actions_executed), json.dumps(result.actions_failed),
             result.skipped_reason, 1 if result.success else 0,
    ...
    def execute(self, reaction: Reaction, event_type: str, event_data: dict) -> ReactionResult:
        if not evaluate_condition(reaction.condition, event_data):
            return ReactionResult(reaction_name=reaction.name, event_type=event_type,
        ...
        for action_name in reaction.actions:
            ...
            try:
                ...
                failed.append(action_name)
        return ReactionResult(reaction_name=reaction.name, event_type=event_type,
                               actions_executed=executed, actions_failed=failed)
    ...
    def get_history(self, limit: int = 50) -> list[dict]:
        ...
        for r in rows:
            ...
            if isinstance(executed, str):
                try: executed = json.loads(executed)
                except Exception: executed = []
            if isinstance(failed, str):
                try: failed = json.loads(failed)
                except Exception: failed = []
            results.append({
                "reaction": r["reaction_name"], "event": r["event_type"],
                "executed": executed, "failed": failed,
                "skipped": r.get("skipped_reason", ""),
                "success": bool(r.get("success", 1)), "timestamp": r["timestamp"],
            })
        return results
    ...
    def stats(self) -> dict:
        ...
        success_row = self.db.fetch_one("SELECT COUNT(*) as cnt FROM reaction_history WHERE success=1")
        ...
        return {
            "total_reactions": total, "successes": successes,
            "failures": total - successes - skipped, "skipped": skipped,
    ...
    def reset_cooldown(self, name: str) -> bool:
        existing = self.db.fetch_one(
            "SELECT reaction_name FROM reaction_cooldowns WHERE reaction_name=?", (name,))
        if existing:
            self.db.execute("DELETE FROM reaction_cooldowns WHERE reaction_name=?", (name,))
            return True
        return False
    ...
    def _action_notify(self, event_type: str, data: dict) -> bool:
        try:
            ...
            if registry.notifiers:
                ...
                event = NotifyEvent(event_type=event_type,
                                     message=data.get("message", f"Reaction triggered: {event_type}"),
                                     details=data, severity=data.get("severity", "warning"))
                for notifier in registry.notifiers:
                    try: notifier.send(event)
    ...
    def _action_log(self, event_type: str, data: dict) -> bool:
        logger.info("AUDIT: event=%s data=%s", event_type,
    ...
    def _action_escalate(self, event_type: str, data: dict) -> bool:
        try:
            from ..events import get_event_bus
            get_event_bus().emit(f"{event_type}.escalated",
...
def get_reaction_engine(config_path: str | Path | None = None, db=None) -> ReactionEngine:
