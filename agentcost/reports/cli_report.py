...
def print_comparison_report(summaries: list[RunSummary]):
    ...
    print()
    print(
        f"  {'Rank':<6}{'Model':<32}{'Quality':<10}{'Income':<12}{'Cost':<12}{'ROI':<10}{'Margin'}"
    )
    ...
    for i, s in enumerate(ranked, 1):
        ...
        print(
            f"  {medal:<6}{s.model:<32}{s.avg_quality:<10.3f}"
    ...
    if all_failed:
        print(
            "  ⚠️  ALL MODELS SCORED ZERO — likely a configuration issue, not a quality issue."
        )
        print("  💡 Common causes:")
        print("     • Wrong model names (check what your proxy exposes)")
        print("     • Invalid API key / virtual key")
        print(
            "     • SSL certificate issue with corporate gateway (try --no-verify-ssl)"
        )
        print("     • Proxy returned errors that were silently caught")
        print("     • Run with a single task first to debug:")
        print(
            "       python -m agentcost benchmark --model <name> --tasks 1 --provider proxy --base-url <url>"
        )
    else:
        # Winner callout
        winner = ranked[0]
        ...
        print(
            f"     Earned ${winner.total_income:.2f} at {winner.avg_quality:.3f} quality"
        )
        print(
            f"     for only ${winner.total_cost:.4f} in LLM costs ({winner.avg_roi:.0f}x ROI)"
        )
    ...
    for s in ranked:
        print(f"  📊 {s.model}")
        print(
            f"     Tasks: {s.completed_tasks}/{s.total_tasks} | "
            f"Tokens: {s.total_input_tokens + s.total_output_tokens:,} | "
            f"Duration: {s.total_duration:.1f}s"
        )
        hourly = (
            (s.total_income / (s.total_duration / 3600)) if s.total_duration > 0 else 0
        )
