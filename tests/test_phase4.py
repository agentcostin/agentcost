...
class TestPluginTypes:
    def test_notifier_plugin(self):
        ...
        class TestNotifier(NotifierPlugin):
            meta = PluginMeta(
                name="test-notifier", version="1.0.0", plugin_type=PluginType.NOTIFIER
            )
        ...
        result = n.send(
            NotifyEvent(
                event_type="test", severity="info", title="Hello", message="World"
            )
        )
    ...
    def test_policy_plugin(self):
        ...
        class TestPolicy(PolicyPlugin):
            meta = PluginMeta(
                name="test-policy", version="1.0.0", plugin_type=PluginType.POLICY
            )
        ...
        p = TestPolicy()
        assert p.evaluate(
            PolicyContext(model="gpt-4o", provider="openai", estimated_cost=1.0)
        ).allowed
        assert not p.evaluate(
            PolicyContext(model="gpt-4o", provider="openai", estimated_cost=10.0)
        ).allowed
    ...
    def test_exporter_plugin(self):
        ...
        class TestExporter(ExporterPlugin):
            meta = PluginMeta(
                name="test-exporter", version="1.0.0", plugin_type=PluginType.EXPORTER
            )
    ...
    def test_provider_plugin(self):
        ...
        class TestProvider(ProviderPlugin):
            meta = PluginMeta(
                name="test-provider", version="1.0.0", plugin_type=PluginType.PROVIDER
            )
...
class TestPluginRegistry:
    def test_load_and_list(self):
        ...
        class MyNotifier(NotifierPlugin):
            meta = PluginMeta(
                name="my-notifier", version="0.1.0", plugin_type=PluginType.NOTIFIER
            )
    ...
    def test_unload(self):
        ...
        class MyPolicy(PolicyPlugin):
            meta = PluginMeta(
                name="temp-policy", version="0.1.0", plugin_type=PluginType.POLICY
            )
    ...
    def test_provider_cost_fallback(self):
        ...
        class MyProvider(ProviderPlugin):
            meta = PluginMeta(
                name="custom-prov", version="0.1.0", plugin_type=PluginType.PROVIDER
            )
...
class TestIntegrationModules:
    ...
    def test_llamaindex_module_exists(self):
        spec = __import__("importlib").util.find_spec(
