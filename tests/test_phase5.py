...
class TestGateway:
    ...
    def test_resolve_provider_openai(self):
        ...
        cfg = GatewayConfig()
        cfg.providers["openai"] = ProviderRoute("openai", "https://api.openai.com/v1", "sk-x")
        assert resolve_provider("gpt-4o", cfg).name == "openai"

    def test_resolve_provider_ollama(self):
        from agentcost.gateway import resolve_provider, GatewayConfig, ProviderRoute
        cfg = GatewayConfig()
        cfg.providers["ollama"] = ProviderRoute("ollama", "http://localhost:11434/v1", "x")
...
def _make_event(cost=0.01, latency=100, tokens=50, status="success"):
    return {"project": "p", "model": "m", "cost": cost,
            "latency_ms": latency, "output_tokens": tokens, "status": status}
...
class TestAnomalyDetection:
    ...
    def test_error_burst(self):
        from agentcost.anomaly import AnomalyDetector, AnomalyType
        d = AnomalyDetector(min_samples=10, error_rate_threshold=0.3, error_rate_window=20)
        for _ in range(10):
    ...
    def test_alert_to_dict(self):
        ...
        a = AnomalyAlert(AnomalyType.COST_SPIKE, Severity.WARNING, "p", "m", "test", 1.0, 0.01, 5.0)
        d = a.to_dict()
    ...
    def test_callback(self):
        from agentcost.anomaly import AnomalyDetector
        received = []
        d = AnomalyDetector(sensitivity=2.0, min_samples=10, on_anomaly=lambda a: received.append(a))
        for _ in range(20):
            d.ingest(_make_event())
...
class TestLangChainIntegration:
    ...
    def test_on_llm_start_tracks_pending(self):
        ...
        cb.on_llm_start({"kwargs": {"model_name": "gpt-4o"}}, ["Hello"], run_id="run-123")
    ...
    def test_on_llm_end_records_trace(self):
        ...
        cb.on_llm_start({"kwargs": {"model_name": "gpt-4o"}}, ["test"], run_id="run-789")

        # Simulate LLMResult-like response
        class FakeResponse:
            llm_output = {"token_usage": {"prompt_tokens": 50, "completion_tokens": 100}}
            generations = []
    ...
    def test_on_llm_error_records_trace(self):
        ...
        cb.on_llm_start({"kwargs": {"model_name": "gpt-4o"}}, ["test"], run_id="run-err")
    ...
    def test_extract_model_from_invocation_params(self):
        ...
        assert _extract_model({}, {"invocation_params": {"model_name": "gpt-4o-mini"}}) == "gpt-4o-mini"

    def test_extract_model_fallback(self):
        from agentcost.sdk.integrations.langchain import _extract_model
        assert _extract_model({"kwargs": {"model": "claude-3-haiku"}}, {}) == "claude-3-haiku"

    def test_extract_tokens_from_llm_output(self):
        from agentcost.sdk.integrations.langchain import _extract_tokens
        class FakeResp:
            llm_output = {"token_usage": {"prompt_tokens": 100, "completion_tokens": 200}}
            generations = []
...
class TestLlamaIndexIntegration:
    ...
    def test_event_end_records_trace(self):
        ...
        h.on_event_end(LLM_EVENT, {"prompt_tokens": 50, "completion_tokens": 100}, event_id="ev-2")
        summary = h.summary()
        assert summary["total_calls"] >= 1
    ...
    def test_extract_tokens_from_payload(self):
        ...
        it, ot = _extract_tokens_llamaindex({"prompt_tokens": 100, "completion_tokens": 200})
        assert it == 100
        assert ot == 200
...
class TestGrafana:
    ...
    def test_references_metrics(self):
        with open(os.path.join(ROOT, "agentcost/otel/grafana-dashboard.json")) as f:
            raw = f.read()
        for metric in ["agentcost_llm_cost_total", "agentcost_llm_calls_total",
                       "agentcost_llm_latency_seconds", "agentcost_budget_utilization"]:
